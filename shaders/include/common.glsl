...
layout(set = 1, binding = IMMUTABLE_SAMPLER_COUNT) uniform textureCube _uTexturesCube[];
...
#define GetTextureCube(Index) \
    _uTexturesCube[nonuniformEXT(Index & TEXTURE_INDEX_TEXTURE_MASK)]
...
#define GetSampledTextureCube(Index) \
    samplerCube(GetTextureCube(Index), _uSamplers[nonuniformEXT(GET_SAMPLER_INDEX(Index))])
