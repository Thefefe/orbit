...
impl EnvironmentMapLoader {
    ...
    pub fn create_from_equirectangular_image(
        ...
        let skybox = context.create_image(format!("{name}_skybox"), &graphics::ImageDesc {
        ...
        context.record_and_submit(|cmd| {
            cmd.barrier(&[], &[
                graphics::image_barrier(&skybox, graphics::AccessKind::None, graphics::AccessKind::ColorAttachmentWrite)
            ], &[]);
